...
export async function getHostAuthenticatedSupabaseClient(): Promise<SupabaseClient | null> {
...
export interface PaymentTransactionRecord {
  ...
  redirect_url?: string;
  instructions?: string;
  method: 'web' | 'ecocash' | 'onemoney' | 'cash';
  created_at: string;
  updated_at?: string;
  verified_at?: string;
  paid_at?: string;
...
async function updateSupabaseBookingPayment(
  ...
  try {
    ...
    const { error: updateErr } = await hostClient
      .from('bookings')
    ...
    const txnRecord: PaymentTransactionRecord = {
      ...
      currency: 'USD',
      status: 'paid',
      method: (resolvedTxn?.method as any) || 'web',
...
async function updateSupabaseServicePayment(
  ...
  try {
    ...
    if (directOrder) {
      ...
      if (allOrders) {
        order = allOrders.find((o: any) => o.id.replace(/-/g, '').toLowerCase().startsWith(cleanRef));
      }
    }
    ...
    // Update service order timestamp in Supabase
    try {
      await hostClient
        .from('service_orders')
        .update({
...
export async function sanitizeExistingBookings() {
  try {
    const hostClient = await getHostAuthenticatedSupabaseClient();
...
export async function sanitizeExistingServiceOrders() {
  try {
    const hostClient = await getHostAuthenticatedSupabaseClient();
    if (!hostClient) return;
